...
class Controller(Node):
    ...
    def __init__(self, name):
        ...
        if self.pub_odom_topic:
            # self.odom_broadcaster = tf2_ros.TransformBroadcaster(self)  # 定义TF变换广播者(define TF transformation broadcaster)
    ...
    def cal_odom_fun(self):
        while True:
            ...
            # 如果velocity是零，说明编码器的误差会比较小，认为编码器数据更可靠(if velocity is zero, it indicates that the encoder error is relatively small, so the encoder data is considered more reliable)
            # 如果velocity非零，考虑到运动中编码器可能带来的滑动误差，认为imu的数据更可靠(if velocity is non-zero, considering the sliding error that the encoder may introduce during motion, IMU data is considered more reliable)
