            ...
            if self.machine_type != 'JetRover_Acker':
                if self.running_mode == 1 and self.timestamp <= time.time():
                    ...
                    if len(min_dist_left_) > 1 and len(min_dist_right_) > 1:
                        ...
                        elif min_dist_left <= self.threshold and min_dist_right <= self.threshold:  # 两侧都有障碍(both sides are blocked)
                            twist.linear.x = self.speed / 6  # 转弯时向前速度尽量小(keep the forward velocity as low as possible when turning)
                            w = self.speed * 6.0  # 转弯速度尽量大(keep the turning speed as high as possible)
                            twist.angular.z = w
                            self.last_act = 3
                        ...
                        elif min_dist_left > self.threshold and min_dist_right <= self.threshold:  # 右侧有障碍(there are obstacles at right)
                            twist.linear.x = self.speed / 6  # 转弯时向前速度尽量小(keep the forward velocity as low as possible when turning)
                ...
                if self.running_mode == 1 and self.timestamp <= time.time():
                    ...
                    if len(min_dist_left_) > 1 and len(min_dist_right_) > 1:
                        ...
                        if self.last_act == 3: 
                            ...
                            twist.angular.z = -twist.linear.x/0.7 # # 右转转弯半径设为0.7m(set the right turn radius to 0.7 meters)
                            self.last_act = 4
                            self.mecanum_pub.publish(twist)
                    ...
                    if len(dist_) > 1:
                        ...
                        data_ = data.copy()
                        u = data_.mean()  # 计算均值(calculate average value)
