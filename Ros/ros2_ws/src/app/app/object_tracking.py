...
class ObjectTracker:
    ...
    def __call__(self, image, result_image, threshold):
        ...
        image = cv2.cvtColor(image, cv2.COLOR_RGB2LAB)  # RGB转LAB空间(convert RGB to LAB)
        ...
        contour_area = list(filter(lambda c: c[1] > 40, contour_area))  # 剔除>面积过小的轮廓(remove contours with areas that are too small)
        circle = None
...
class OjbectTrackingNode(Node):
    ...
    def main(self):
        while True:
            ...
            if self.debug and not self.set_callback:
                ...
                # 设置鼠标点击事件的回调函数(set callback function for mouse click events)
    ...
    def image_callback(self, ros_image):
        ...
        result_image = np.copy(rgb_image)  # 显示结果用的画面(display image for the result)
        with self.lock:
            # 颜色拾取器和识别追踪互斥, 如果拾取器存在就开始拾取(color picker and recognition tracking are mutually exclusive. If the picker exists, start picking)
            if self.color_picker is not None:  # 拾取器存在(the picker exists)
