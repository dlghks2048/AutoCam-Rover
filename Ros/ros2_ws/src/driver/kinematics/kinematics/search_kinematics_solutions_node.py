...
# @data:2023/03/20
# @author:aiden
# 实时获取角度反馈，根据当前位置和(real-time angle feedback retrieval, obtaining the optimal solution based on the minimum difference between the current position and the target position)
# 目标位置的最小差值来获取最优解
import rclpy
import numpy as np
...
fk = ForwardKinematics(debug=False)  # 不开启打印(without enabling printing)
class SearchKinematicsSolutionsNode(Node):
    ...
    def get_link_srv(self, request, response):
        # 获取各个link长度(obtain each link length)
        data = get_link()
        data1 = fk.get_link()
        link = Link()
    ...
    def get_joint_range_srv(self, request, response):
        # 获取各个关节范围(obtain each joint range)
    ...
    def set_joint_value_target_srv(self, request, response):
        # 正运动学解(direct kinematics solution)
    ...
    def get_servo_position(self, msg):
        # 获取舵机当前角度(get the current angle of the servo)
        servo_states = []
    ...
    def set_pose_target(self, position, pitch, pitch_range, resolution):
        # 逆运动学解，获取最优解(所有电机转动最小)(inverse kinematic solution, obtaining the optimal solution (minimizing rotation of all motors))
