...
# 连杆长度(m)(link length (meters))
# 底座的高度，这里把第一个坐标系和第二个坐标的原点重合到一起了(the height of the base, where the origin of the first coordinate system is aligned with the origin of the second coordinate system)
...
# 各关节角度限制，取决于是否碰撞以及舵机的转动范围(the joint angle limits depend on whether there is collision and the range of motion of the servos)
# 多加0.2为了防止计算时数值的不稳定，会比设定值大一点点(add additional 0.2 to prevent numerical instability during calculations, making it slightly larger than the set value)
joint1 = [-120.2, 120.2]
joint2 = [-180.2, 0.2]
joint3 = [-120.2, 120.2]
joint4 = [-200.2, 20.2]
...
#         舵机脉宽范围，中位值，对应的角度范围，中位值(servo pulse width range, median value, corresponding angle range, median value)
...
# 旋转矩阵--->欧拉角(rotation matrix to Euler angles)
def rot2rpy(R):
