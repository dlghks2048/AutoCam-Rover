...
class PacketFunction(enum.IntEnum):
    ...
    PACKET_FUNC_PWM_SERVO = 4  # PWM舵机控制, 板子上从里到外依次为1-4(PWM servo control, numbered 1 to 4 from inside to outside on the board)
...
class Board:
    ...
    def get_battery(self):
        # 获取电压，单位mAh(get voltage, unit mAh)
        if self.enable_recv:
            try:
    ...
    def get_imu(self):
        # 获取IMU数据，返回ax, ay, az, gx, gy, gz(get IMU data, return ax, ay, az, gx, gy, gz)
        if self.enable_recv:
            try:
                # ax, ay, az, gx, gy, gz
