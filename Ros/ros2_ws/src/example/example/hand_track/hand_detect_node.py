...
class HandDetectNode(Node):
    def __init__(self, name):
        ...
        self.result_publisher = self.create_publisher(Image, '~/image_result', 1)   # 图像处理结果发布(publishing of image processing results)
        self.image_sub = self.create_subscription(Image, '/%s/rgb/image_raw' % self.camera, self.image_callback, 1)  # 摄像头订阅(camera subscription)
