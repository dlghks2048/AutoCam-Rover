...
class AutomaticPickNode(Node):
    ...
    def color_detect(self, img):
        ...
        if len(contours) != 0:
            areaMaxContour, area_max = common.get_area_max_contour(contours, 10)  # 找出最大轮廓(find out the contour with the maximal area)
            if areaMaxContour is not None:
                #print(111111111111, area_max)
                if 10 < area_max:  # 有找到最大面积(the maximal area is found)
                    ...
                    if self.machine_type == 'JetRover_Mecanum':
                        ...
                        if self.count_turn > 5:
                            ...
                            if self.count_stop < 10:  # 连续10次都没在移动(hasn't moved for 10 consecutive times)
                                if object_angle < 40: # 不取45，因为如果在45时值的不稳定会导致反复移动(exclude 45, as instability in values at this angle may lead to repetitive movements)
                                    object_angle += 90
